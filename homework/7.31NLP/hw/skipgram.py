    ...
    def __init__(self, corpus, vocab, context_size=2):
        ...
        for sentence in tqdm(corpus, desc="Dataset Construction"):
            ...
            for i in range(1, len(sentence)-1):
                ...
                self.data.extend([(w, c) for c in context])      # 根据w输出一定窗口内的c
...
class SkipGramModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim):
        ...
        self.embeddings = nn.Embedding(vocab_size, embedding_dim)   # embedding e
        self.output = nn.Linear(embedding_dim, vocab_size)    # 线性变换
        init_weights(self)

    def forward(self, inputs):
        embeds = self.embeddings(inputs)
        output = self.output(embeds)
        log_probs = F.log_softmax(output, dim=1)       # softmax
        return log_probs
...
for epoch in range(num_epoch):
    ...
    for batch in tqdm(data_loader, desc=f"Training Epoch {epoch}"):
        ...
        loss = nll_loss(log_probs, targets)  # loss function
        loss.backward()
        optimizer.step()
